...
-- -----------------------------------------------------
-- Table `EzHRM`.`BANGPHAT`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `EzHRM`.`BANGPHAT` ;

CREATE TABLE IF NOT EXISTS `EzHRM`.`BANGPHAT` (
  `TenPhat` VARCHAR(255) NOT NULL,
  `LyDoPhat` TEXT NOT NULL,
  `SoTienPhat` DECIMAL NOT NULL,
  `SoPhanTramLuongPhat` FLOAT NOT NULL,
  PRIMARY KEY (`TenPhat`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `EzHRM`.`CHAMCONG` (
  ...
  `IDNhienVien` VARCHAR(10) NOT NULL,
  `ThoiGianVaoLam` TIME NOT NULL,
  `ThoiGianTanLam` TIME NOT NULL,
  PRIMARY KEY (`ID`),
  CONSTRAINT `FK_CHAMCONG_IDNHANVIEN_ID`
    FOREIGN KEY (`IDNhienVien`)
    REFERENCES `EzHRM`.`NHANVIEN` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `FK_CHAMCONG_IDNHANVIEN_ID_idx` ON `EzHRM`.`CHAMCONG` (`IDNhienVien` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `EzHRM`.`CHUCVU` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `TenChucVu` MEDIUMTEXT NOT NULL,
  `CachTinhLuong` MEDIUMTEXT NOT NULL,
  `MucLuongNgoaiGio` FLOAT NOT NULL,
  PRIMARY KEY (`ID`),
...
-- -----------------------------------------------------
-- Table `EzHRM`.`DONVICONGVIEC`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `EzHRM`.`DONVICONGVIEC` ;

CREATE TABLE IF NOT EXISTS `EzHRM`.`DONVICONGVIEC` (
  `Ngay` DATE NOT NULL,
  `IDNhanVien` VARCHAR(10) NOT NULL,
  `SoGioLamTrongGio` INT NOT NULL DEFAULT 0,
  `SoGioLamNgoaiGio` INT NOT NULL DEFAULT 0,
  `SoSanPhamNgoaiGio` INT NOT NULL DEFAULT 0,
  `SoHopDongThoaThuan` INT NOT NULL DEFAULT 0,
...
CREATE INDEX `ID_idx` ON `EzHRM`.`DONVICONGVIEC` (`IDNhanVien` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `EzHRM`.`LUONG` (
  ...
  PRIMARY KEY (`NgayTraLuong`, `IDNhanVien`),
...
CREATE TABLE IF NOT EXISTS `EzHRM`.`NHANVIEN` (
  ...
  PRIMARY KEY (`ID`),
  CONSTRAINT `FK_NHANVIEN_PHONGBAN_ID`
    FOREIGN KEY (`PhongBan`)
    REFERENCES `EzHRM`.`PHONGBAN` (`TruongPhong`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_NHANVIEN_CHUCVU_ID`
    FOREIGN KEY (`ChucVu`)
    REFERENCES `EzHRM`.`CHUCVU` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE INDEX `FK_NHANVIEN_PHONGBAN_ID_idx` ON `EzHRM`.`NHANVIEN` (`PhongBan` ASC) VISIBLE;

CREATE INDEX `FK_NHANVIEN_CHUCVU_ID_idx` ON `EzHRM`.`NHANVIEN` (`ChucVu` ASC) VISIBLE;
...
-- -----------------------------------------------------
-- Table `EzHRM`.`QUYENHAN`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `EzHRM`.`QUYENHAN` ;

CREATE TABLE IF NOT EXISTS `EzHRM`.`QUYENHAN` (
  `Bitmask` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `EzHRM`.`THAMSO` (
  ...
  `ThoiGianChoPhepDiTre` TINYINT NOT NULL COMMENT 'In minutes cause store this in time would be weird',
  `ThoiGianChoPhepVeSom` TINYINT NOT NULL COMMENT 'In minutes',
...
CREATE TABLE IF NOT EXISTS `EzHRM`.`THOIGIANBIEUTUAN` (
  `ThoiDiemTao` DATETIME NOT NULL,
  `GioVaoLamThuHai` TIME NOT NULL,
  `GioVaoLamThuBa` TIME NOT NULL,
  `GioVaoLamThuTu` TIME NOT NULL,
  `GioVaoLamThuNam` TIME NOT NULL,
  `GioVaoLamThuSau` TIME NOT NULL,
  `GioVaoLamThuBay` TIME NOT NULL,
  `GioVaoLamChuNhat` TIME NOT NULL,
  `GioTanLamThuHai` TIME NOT NULL,
  `GioTanLamThuBa` TIME NOT NULL,
  `GioTanLamThuTu` TIME NOT NULL,
  `GioTanLamThuNam` TIME NOT NULL,
  `GioTanLamThuSau` TIME NOT NULL,
  `GioTanLamThuBay` TIME NOT NULL,
  `GioTanLamThuChuNhat` TIME NOT NULL,
  PRIMARY KEY (`ThoiDiemTao`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `EzHRM`.`THONGTINTINHLUONG`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `EzHRM`.`THONGTINTINHLUONG` ;

CREATE TABLE IF NOT EXISTS `EzHRM`.`THONGTINTINHLUONG` (
  `ThoiDiemTao` DATE NOT NULL,
  `IDNhanVien` VARCHAR(7) NOT NULL,
  `TienLuongMoiGio` DECIMAL NOT NULL,
  `TienLuongMoiThang` DECIMAL NOT NULL,
  `TienLuongTrenMoiSanPham` DECIMAL NOT NULL,
  `TienHoaHongTrenMoiHopDong` DECIMAL NOT NULL,
  `TienLuongCungMoiThang` DECIMAL NOT NULL,
  PRIMARY KEY (`ThoiDiemTao`, `IDNhanVien`),
  CONSTRAINT `FK_TINHLUONG_IDNHANVIEN_ID`
    FOREIGN KEY (`IDNhanVien`)
    REFERENCES `EzHRM`.`NHANVIEN` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `ID_idx` ON `EzHRM`.`THONGTINTINHLUONG` (`IDNhanVien` ASC) VISIBLE;
