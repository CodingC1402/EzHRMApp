...
CREATE TABLE IF NOT EXISTS `EzHRM`.`CHUCVU` (
  ...
  CONSTRAINT `FK_CHUCVU_CACHTINHLUONG_TEN`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_CHUCVU_NHOMTAIKHOAN1`
...
CREATE INDEX `fk_CHUCVU_NHOMTAIKHOAN1_idx` ON `EzHRM`.`CHUCVU` (`NhomTaiKhoan` ASC) VISIBLE;
...
CREATE UNIQUE INDEX `EmailVanPhong_UNIQUE` ON `EzHRM`.`NHANVIEN` (`EmailVanPhong` ASC) VISIBLE;

CREATE UNIQUE INDEX `EmailCaNhan_UNIQUE` ON `EzHRM`.`NHANVIEN` (`EmailCaNhan` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `EzHRM`.`SOGIOLAMTRONGNGAY` (
  `Ngay` DATE NOT NULL,
  `IDNhanVien` VARCHAR(10) NOT NULL,
  `SoGioLamTrongGio` FLOAT NOT NULL DEFAULT 0,
  `SoGioLamNgoaiGio` FLOAT NOT NULL DEFAULT 0,
